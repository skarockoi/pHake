...
bool ReadSignatures()
{
	...
	auto t5 = std::async(std::launch::async, ([]() { settings->pointers.asm_update_position = process->FindSignature({ 0x0F, 0x29, 0x48, 0x00, 0x48, 0x83, 0xC4, 0x00, 0x5B, 0xC3, 0xCC }); }));
	...
	auto t7 = std::async(std::launch::async, ([]() { settings->pointers.kmh = process->ReadOffsetFromSignature<uint32_t>({ 0xF3, 0x0F, 0x10, 0x05, 0x00 ,0x00, 0x00, 0x00, 0xC6, 0x85 }, 4); }));
